...
public class HDFSBlockLayoutColumnarLongsSupplier
        implements Supplier<ColumnarLongs>
{
    private final HDFSGenericIndexed<ResourceHolder<ByteBuffer>> baseLongBuffers;
    ...
    public HDFSBlockLayoutColumnarLongsSupplier(
            int totalSize,
            int sizePer,
            HDFSByteBuff fromBuffer,
            ByteOrder order,
            CompressionFactory.LongEncodingReader reader,
            CompressionStrategy strategy)
            throws IOException
    {
        baseLongBuffers = HDFSGenericIndexed
                .read(fromBuffer, new HDFSDecompressingByteBufferObjectStrategy(order, strategy));
    ...
    @Override public ColumnarLongs get()
    {
        ...
        if (isPowerOf2) {
            ...
            if (baseReader instanceof LongsLongEncodingReader) {
                return new HDFSBlockLayoutColumnarLongsSupplier.BlockLayoutColumnarLongs()
                {
                    ...
                    @Override protected void loadBuffer(int bufferNum)
                    {
            ...
            implements ColumnarLongs
    {
        ...
        @Override public void get(final long[] out, final int start, final int length)
        {
            // division + remainder is optimized by the compiler so keep those together
        ...
        @Override public void get(final long[] out, final int[] indexes, final int length)
        {
            int p = 0;
            ...
            while (p < length) {
                ...
                final int numRead =
                        reader.read(out, p, indexes, length - p, bufferNum * sizePer, sizePer);
                //assert numRead > 0;
                p += numRead;
            }
        }
        ...
        {
            return "HDFSBlockCompressedColumnarLongs_Anonymous{"
