...
public class HDFSBlockLayoutColumnarDoublesSupplier
        implements Supplier<ColumnarDoubles>
{
    private final HDFSGenericIndexed<ResourceHolder<ByteBuffer>> baseDoubleBuffers;
    ...
    public HDFSBlockLayoutColumnarDoublesSupplier(int totalSize, int sizePer,
            HDFSByteBuff fromBuffer, ByteOrder byteOrder, CompressionStrategy strategy)
            throws IOException
    {
        baseDoubleBuffers = HDFSGenericIndexed.read(fromBuffer,
                new HDFSDecompressingByteBufferObjectStrategy(byteOrder, strategy));
        this.totalSize = totalSize;
        this.sizePer = sizePer;
    }

    @Override public ColumnarDoubles get()
    {
            ...
            implements ColumnarDoubles
    {
        ...
        @Override public void get(final double[] out, final int start, final int length)
        {
            // division + remainder is optimized by the compiler so keep those together
        ...
        @Override public void get(final double[] out, final int[] indexes, final int length)
        {
            int p = 0;
        ...
        {
            return "HDFSBlockCompressedColumnarDoubles_Anonymous{"
                    + "currBufferNum="
                    + currBufferNum
                    + ", sizePer=" + sizePer + ", numChunks="
                    + singleThreadedDoubleBuffers.size()
                    + ", totalSize=" + totalSize + '}';
        }
    }
}
