...
public class HDFSBlockLayoutColumnarFloatsSupplier
        implements Supplier<ColumnarFloats>
{
    private final HDFSGenericIndexed<ResourceHolder<ByteBuffer>> baseFloatBuffers;
    ...
    public HDFSBlockLayoutColumnarFloatsSupplier(int totalSize, int sizePer,
            HDFSByteBuff fromBuffer, ByteOrder byteOrder, CompressionStrategy strategy)
            throws IOException
    {
        baseFloatBuffers = HDFSGenericIndexed.read(fromBuffer,
            ...
            implements ColumnarFloats
    {
        final Indexed<ResourceHolder<ByteBuffer>> singleThreadedFloatBuffers =
                baseFloatBuffers.singleThreaded();
        ...
        @Override public void get(final float[] out, final int start, final int length)
        {
            // division + remainder is optimized by the compiler so keep those together
        ...
        @Override public void get(final float[] out, final int[] indexes, final int length)
        {
            int p = 0;
        ...
        {
            return "HDFSBlockCompressedColumnarFloats_Anonymous{"
