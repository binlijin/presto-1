...
import io.prestosql.cache.CacheStats;
        ...
        implements CacheManager
{
    ...
    private static final int FILE_MERGE_BUFFER_SIZE = toIntExact(DataSize.of(8, MEGABYTE).toBytes());
    ...
    private boolean read(FileReadRequest request, byte[] buffer, int offset)
    {
        ...
        readLock.lock();
        try {
            Map<Range<Long>, LocalCacheFile> diskRanges = cacheRange.getRange().subRangeMap(
                    Range.closedOpen(request.getOffset(), request.getLength() + request.getOffset())).asMapOfRanges();
    ...
    private boolean write(FileReadRequest key, byte[] data, Path newFilePath)
    {
        ...
        try {
            ...
            if (!cacheFileEquals(previousCacheFile, newPreviousCacheFile) || !cacheFileEquals(followingCacheFile, newFollowingCacheFile)) {
                ...
                cacheFilesToDelete = cache.subRangeMap(
                        Range.closedOpen(key.getOffset(), key.getOffset() + key.getLength())).asMapOfRanges().values().stream()
