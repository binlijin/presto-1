...
import io.prestosql.druid.DataInputSource;
import io.prestosql.druid.zip.CentralDirectoryFileHeader;
import io.prestosql.druid.zip.EndOfCentralDirectoryRecord;
import io.prestosql.druid.zip.Zip64EndOfCentralDirectory;
import io.prestosql.druid.zip.Zip64EndOfCentralDirectoryLocator;
import io.prestosql.druid.zip.ZipFileData;
import io.prestosql.druid.zip.ZipFileEntry;
import io.prestosql.druid.zip.ZipUtil;
            ...
            if (fileData.isMaybeZip64()) {
                try {
                    Zip64EndOfCentralDirectoryLocator
                            .read(fileData, dataInputSource, centralDirectoryEndOffset - Zip64EndOfCentralDirectoryLocator.FIXED_DATA_SIZE);
            ...
            throws IOException
    {
        try {
            ...
            for (long i = 0; i < count; i++) {
                position += CentralDirectoryFileHeader
                        .read(fileData, dataInputSource, position, charset);
            }
        }
        catch (ZipException e) {
            throw new PrestoException(DRUID_SEGMENT_LOAD_ERROR, e);
