...
public class LongColumnReaderv2
        implements ColumnReader
{
    ...
    public LongColumnReaderv2(Offset offset, BaseColumn baseColumn, DimFilter postFilter)
    {
        ...
        if (offset instanceof SimpleReadableOffset) {
            baseOffset =
                    new NoFilterVectorOffset(((SimpleReadableOffset) offset).getMaxBatchSize(), 0, ((SimpleReadableOffset) offset).getRowCount());
            vectorValueSelector = baseColumn.makeVectorValueSelector(baseOffset);
        }
    }
    ...
    public Block readBlock(Type type, int batchSize, boolean filterBatch)
    {
        if (vectorValueSelector != null) {
            ...
            if (filterBatch) {
                ...
                long[] longVectors = vectorValueSelector.getLongVector();
                baseOffset.advance();
                for (int i = 0; i < batchSize; i++) {
                    long value = longVectors[i];
                    type.writeLong(builder, value);
                    if (!filterBatch) {
